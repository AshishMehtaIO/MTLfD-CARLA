...
def main():
    ...
    if args.sample_train:
        train_batch = D.generate_train_batch_sample(batchSize, 1000)
    else:
        train_batch = D.generate_train_batch(batchSize)
    ...
    while epoch <= epochs:
        ...
        trainX, trainY, epoch, train_index = train_batch.__next__()
        measurements_train = trainY[:, 0:3]
        speed_vec_train = trainY[:, 10:11] / 35
        highlevel_command_train = get_one_hot(trainY[:, 24:25] - 2, 4)
        action_abstraction_train = sparse_action_visual_abstraction(trainY[:, 25])
        visual_abstraction_train = trainY[:, 34:47]

        _, train_loss, train_auxiliary_loss, train_main_loss, = \
            sess.run([train_op, loss, weighted_auxiliary_losses, main_loss],
                     feed_dict={input_im: trainX,
                                auxiliary_visual_y: visual_abstraction_train,
                                auxiliary_action_y: action_abstraction_train,
                                driving_y: measurements_train,
                                speed: speed_vec_train,
                                planner: highlevel_command_train,
                                learning_rate: lr})

        total_train_loss = total_train_loss + train_loss
        total_train_auxiliary_loss = total_train_auxiliary_loss + train_auxiliary_loss
        total_train_main_loss = total_train_main_loss + train_main_loss
        avg_train_loss = total_train_loss / step_in_epoch_train
        avg_train_auxiliary_loss = total_train_auxiliary_loss / step_in_epoch_train
        avg_train_main_loss = total_train_main_loss / step_in_epoch_train

        print("Epoch: ", epoch, "\tTraining Loss: ", train_loss, "\tAvgTraining Loss: ", avg_train_loss,
              "\tTraining Auxiliary Loss: ", train_auxiliary_loss, "\tAvgTraining Auxiliary Loss: ",
              avg_train_auxiliary_loss, "\tTraining Main Loss: ", train_main_loss,
              "\tAvgTraining Main Loss: ", avg_train_main_loss, '\n')
        ...
        if step % 10 == 0:
            ...
            val_loss, val_auxiliary_loss, val_main_loss, summary = \
                sess.run([loss, weighted_auxiliary_losses, main_loss, merged],
