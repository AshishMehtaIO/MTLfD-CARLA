...
float prob_brake = 0.00005;
volatile float brake_noise[num_vehicles];
long brake_noise_duration[num_vehicles]; 
volatile float steer_noise[num_vehicles];
bool brake_noise_flag[num_vehicles];
volatile float rd_num[num_vehicles];
long brake_noise_tick_count[num_vehicles];
long long tick=0;
int car_num=0;

size_t memory_addr[num_vehicles];
...
void AWheeledVehicleAIController::TickAutopilotController()
{
  ...
  const auto Speed = Vehicle->GetVehicleForwardSpeed() * 0.036f;
   ...
   //car_num = tick % num_vehicles;
  if (tick<num_vehicles)
    memory_addr[tick] = (size_t)Vehicle;
  else
  { 
    for (int i=0;i<num_vehicles;i++)
      {
        if (memory_addr[i]==(size_t)Vehicle)
        {
          car_num=i;
          break;
        }
  ...
  for (int counter_var=0; counter_var<num_vehicles; counter_var++)
    {
     rd_num[counter_var] = ((double) rand() / (RAND_MAX)); 

      if (rd_num[counter_var] > prob_brake && (brake_noise_flag[counter_var])==false)
        {
          brake_noise[counter_var]=0.0;
          brake_noise_flag[counter_var]=false;
          brake_noise_duration[counter_var] = 500 + (rand() % static_cast<int>(10000 - 500 + 1));
        }
      else
      {
        brake_noise_tick_count[counter_var]+=1;
        brake_noise_flag[counter_var]=true;
        if (brake_noise_tick_count[counter_var] < brake_noise_duration[counter_var])
          brake_noise[counter_var]=1.0;
        else if (brake_noise_tick_count[counter_var]>= brake_noise_duration[counter_var])
        {
          brake_noise_flag[counter_var]=false;
          brake_noise_tick_count[counter_var]=0;
        }
      }
    }
  ...
  //steer_noise[0] = 1.0f; 
   ...
   std::cout<<"Car speed:\t"<<Speed<<"\tbrake_noise_flag\t"<<brake_noise_flag[car_num]<<std::endl; 
  for(int j=0;j<num_vehicles;j++)
    {
      if (rd_num[j]<=prob_brake)
        std::cout<<rd_num[j]<<'\t';
    }
  std::cout<<"\n"; 
  ...
  if (Throttle < 0.001f) {
    ...
    AutopilotControl.Brake = brake_noise[car_num];
    AutopilotControl.Throttle = Throttle - brake_noise[car_num]*Throttle;
  }
